...
def get_page_url(file=FILE_LOC, remove_str=REMOVE_STR) -> list:
    ...
    df["Meme Page URL"] = [
        items.replace(remove_str, "") if items.startswith(remove_str) else items
        for items in df["Meme Page URL"]
    ]
...
pd.DataFrame(url_list).to_csv(
    url_stores / "image_urls.csv"
)  # updates per run of the program
